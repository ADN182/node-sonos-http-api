...
const spotifyDef = {
  ...
  metastart: {
               ...
               song:     '100032020spotify%3atrack%3a',
               station:  '1000c206cspotify%3aartistRadio%3a',
               playlist: '1006206cspotify%3aplaylist%3a',
             },
  parent:    {
               album:    '00020000album:',
               song:     '00020000track:',
               station:  '10052064spotify%3aartist%3a',
...
function getURI(type, id) {
  if (type == 'album') {
    console.log('album', `x-rincon-cpcontainer:1004206cspotify%3aalbum%3a${id}`)
    return `x-rincon-cpcontainer:1004206cspotify%3aalbum%3a${id}`;
  } else 
  if (type == 'song') {
    console.log('song', `x-sonos-spotify:spotify%3atrack%3a${id}?sid=${sid}&flags=8224&sn=${accountSN}`)
    return `x-sonos-spotify:spotify%3atrack%3a${id}?sid=${sid}&flags=8224&sn=${accountSN}`;
  } else
  if (type == 'station') {
    console.log('station', `x-sonosapi-radio:spotify%3aartistRadio%3a${id}?sid=${sid}&flags=8300&sn=${accountSN}`)
    return `x-sonosapi-radio:spotify%3aartistRadio%3a${id}?sid=${sid}&flags=8300&sn=${accountSN}`;
  } else
  if (type == 'playlist') {
    console.log('playlist', `x-rincon-cpcontainer:1006206cspotify%3aplaylist%3a${id}?sid=${sid}&flags=8300&sn=${accountSN}`)
    return `x-rincon-cpcontainer:1006206cspotify%3aplaylist%3a${id}?sid=${sid}&flags=8300&sn=${accountSN}`;
  }
}
...
function getMetadata(type, id, name, title) {
  ...
  console.log('metedata', `<DIDL-Lite xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:upnp="urn:schemas-upnp-org:metadata-1-0/upnp/"
...
function getURIandMetadata(type, resList)
{
  ...
  UaM.uri = getURI(type, encodeURIComponent(items[0].id));
